...
import net.majorkernelpanic.streaming.heart.HeartRateStream;
import net.majorkernelpanic.streaming.heart.HeartStream;
...
public class SessionBuilder {
	...
	/** Can be used with {@link #setHeartEncoder}. */
	public final static int HEART_NONE= 0;
	...
	private int mHeartEncoder = HEART_RATE;
	...
	public Session build() throws IOException {
		...
		switch (mHeartEncoder) {
		case HEART_RATE:
			session.addHeartTrack(new HeartRateStream());
			break;
		}
		...
		if (session.getHeartTrack()!=null) {
			HeartStream heart = session.getHeartTrack();
			heart.setDestinationPorts(5002);
		}
	...
	/** Sets the default video encoder. */
	public SessionBuilder setHeartEncoder(int encoder) {
		mHeartEncoder = encoder;
		return this;
	}
